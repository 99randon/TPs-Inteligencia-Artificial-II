...
class Genetico:
    # Constructor
    def __init__(self, filas, columnas, estantes):
        self.poblacion = np.zeros((16, estantes))
        self.filas = filas
        self.columnas = columnas
        self.inicio = 1
        self.fin = 1
        self.seleccionados = np.zeros((4, estantes))
        self.costo_sel = []
    ...
    def PoblacionAleatoria(self):
        for i in range(0 , len(self.poblacion)):
            self.poblacion[i] = random.sample(range(1, self.estantes+1), self.estantes)
    ...
    def cruce(self):
        nueva_generacion = np.zeros((12, self.estantes))
        k2 = 999
        k1 = 0
        ...
        for k in range(0, len(nueva_generacion), 2):
            ...
            # Cruce de Orden
            j = cut2 + 1
            for i in range(cut2, len(nueva_generacion[k+1])-1): #idx_2
                if j == len(hijo2):
                    j = 0
                if nueva_generacion[k+1][i+1] not in hijo2:
                    hijo2[j] = nueva_generacion[k+1][i+1]
                    j += 1
                    if j == len(hijo2):
                        j = 0
            l = cut2 + 1
            for i in range(cut2, len(nueva_generacion[k])-1):
                if l == len(hijo1):
                    l = 0
                if nueva_generacion[k][i+1] not in hijo1:
                    hijo1[l] = nueva_generacion[k][i+1]
                    l += 1
                    if l == len(hijo1):
                        l = 0
...
if __name__ == "__main__":
    ...
    filas = 2
    columnas = 3
    estantes = filas*columnas*8
    order_1 = [22, 24, 25, 27, 29, 31, 33, 46, 47]
    order_2 = [12, 20, 24, 25, 27, 29, 33, 35, 37, 38, 42]
    order_3 = [21, 23, 24, 25, 39, 40, 44]
    order_4 = [1, 8, 17, 20, 25, 30, 31, 36, 37, 41, 45]
    order_5 = [20, 21, 25, 29, 36]
    order_6 = [3, 20, 30, 32, 33, 38, 47, 48]
    order_7 = [8, 21, 39, 42, 48]
    order_8 = [5, 15, 20, 23, 24, 39, 41, 42]
    order_9 = [13, 23, 32, 44]
    order_10 = [20, 25, 33, 35, 41, 43, 47]
    ordenes = [order_1, order_2, order_3, order_4, order_5, order_6, order_7, order_8, order_9, order_10]

    k = 0
    g = Genetico(filas, columnas, estantes)
    ...
    while 200>k:
        ...
        print(f"Generacion: {k}\t\tCosto: {costo[0]} \tCosto total: {sum(costo_t)}")
